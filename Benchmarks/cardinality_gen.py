...
def trans_multi_dual_vnnlib(spec_path, queries, testset, safe=True):
    ...
    for index in range(SIZE):
        ...
        with open(spec_path%index, "w") as f:
            ...
            for tt in range(len(all_tensor)):
                ...
                for i, j in enumerate(all_tensor[tt]):
                    if i in flex_index:
                        f.write(f"(assert (>= X_{i} {round(min(j.item() + 0.0001, 1.0), 16)}))\n")
                        f.write(f"(assert (<= X_{i} {1.0}))\n")
                    else:
                        f.write(f"(assert (>= X_{i} {round(j.item(), 16)}))\n")
                        f.write(f"(assert (<= X_{i} {round(j.item(), 16)}))\n")

                for i, j in enumerate(all_tensor[tt]):
                    if i in flex_index:
                        f.write(f"(assert (>= X_{i + 154} {0.0}))\n")
                        f.write(f"(assert (<= X_{i + 154} {round(max(0.0, j.item() - 0.0001), 16)}))\n")
                    else:
                        f.write(f"(assert (>= X_{i + 154} {round(j.item(), 16)}))\n")
                        f.write(f"(assert (<= X_{i + 154} {round(j.item(), 16)}))\n")
                if flag == 9:
                    f.write("(assert (>= Y_0 1e-5))\n")
                elif flag == 11:
                    f.write("(assert (<= Y_0 -1e-5))\n")
                else:
...
def trans_multi_vnnlib(spec_path, queries, doppel_queries, label_range, testset, safe=True):
    ...
    for index in range(SIZE):
        with open(spec_path % (1, index), "w") as f:
            f.write("\n")
            ...
            for _i in range(len(queries)):
                ...
                for ri, r in enumerate(sub_label_range):
                    ...
                    for i, (j, k) in enumerate(zip(tensor, dopple_tensor)):
                        f.write(f"(assert (>= X_{i} {round(min(j, k).item(), 16)}))\n")
                        f.write(f"(assert (<= X_{i} {round(max(j, k).item(), 16)}))\n")

                    f.write(f"(assert (<= Y_0 {round(normalize_labels([r], 0.0, 19.94772801931604)[0][0], 16)}))\n")

        with open(spec_path % (2, index), "w") as f:
            f.write("\n")
            for i in range(all_tensor[0].shape[0]):
                f.write(f"(declare-const X_{i} Real)\n")
            f.write(f"(declare-const Y_0 Real)\n")
        ...
        for size in single_difficulties:
            ...
            for sf, indexes in enumerate(np.load(f'src/mscn/mscn_resources/sf_list_{model}.npy', allow_pickle=True)):
                ...
                try:
                    ...
                    continue
                trans_multi_vnnlib(f'{dir_path}/cardinality_{model}_%d_%d.vnnlib',
        ...
        for size in dual_difficulties:
            for sf, indexes in enumerate(
                ...
                try:
                    ...
                    continue
                trans_multi_dual_vnnlib(f'{dir_path}/cardinality_{model}_dual_%d.vnnlib',
