...
NN_MODELS = ['./results/nn_model_ep_BEST_small_linear_21900.pth','./results/nn_model_ep_BEST_mid_linear.pth' ,'./results/nn_model_ep_BEST_big_linear_49500.pth' ]
    ...
    for MODEL_ptr in range(len(MODEL_LIST)):
        for MODEL_TYPE in MODEL_TYPES:
            ...
            if MODEL_TYPE == 'concat':
                if MODEL == 'mid':
                    actor = model.ActorNetwork_mid_concat(state_dim=[S_INFO, S_LEN], action_dim=A_DIM,
                                                          learning_rate=ACTOR_LR_RATE)
                if MODEL == 'big':
                    actor = model.ActorNetwork_big_concat(state_dim=[S_INFO, S_LEN], action_dim=A_DIM,
                                                          learning_rate=ACTOR_LR_RATE)
                if MODEL == 'small':
                    actor = model.ActorNetwork_small_concat(state_dim=[S_INFO, S_LEN], action_dim=A_DIM,
                                                            learning_rate=ACTOR_LR_RATE)
